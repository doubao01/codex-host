...
export function createRuntimeSessionContext(input: {
  ...
  session: Pick<HarnessSession, "initialState" | "capabilities">;
  ...
  const modelId =
    input.session.initialState.effectiveModel?.id ?? input.record.transportModelId;
  if (!modelId) throw new Error(`Runtime Session has no Model identity: ${input.record.hostThreadId}`);
