...
describe("Grok local media Markdown", () => {
  // Path assertions are POSIX-shaped (/Users/chris/.grok); path.resolve rewrites
  // them on Windows, so these run on POSIX hosts only.
  it.skipIf(process.platform === "win32")(
    "rewrites session-relative video images to absolute paths Codex can play",
    () => {
      expect(
  ...
  it.skipIf(process.platform === "win32")(
    "turns Grok backtick video paths into Markdown images",
    () => {
  ...
  it.skipIf(process.platform === "win32")(
    "holds an unfinished image destination so streamed deltas stay prefix-stable",
    () => {
  ...
  it.skipIf(process.platform === "win32")(
    "holds an unfinished backtick path so streamed deltas stay prefix-stable",
    () => {
      let emitted = "";
      const chunks = ["影片：\n\n`", "videos/usegrokbot-templates-promo-10s.mp4", "`\n完成"];
      let raw = "";
